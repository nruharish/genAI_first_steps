...
# Other tools (create_promotion, create_product_class, etc.) remain the same as before
@tool
...
# ----------------------------
# 8. Assistant loop with audio
# ----------------------------
if __name__ == "__main__":
    print("Siebel Product Model Assistant (type 'exit' or 'quit' to stop)")
    state = {}

    use_audio = input("Use audio input/output? (y/n): ").strip().lower() == "y"

    while True:
        user_input = get_voice_input() if use_audio else input("You: ")
        if not user_input:
            continue
        ...
        while True:
            ...
            if result.startswith("MISSING_FIELDS:"):
                ...
                for field in fields:
                    if use_audio:
                        speak_output(f"Please provide {field}")
                        value = get_voice_input()
                    else:
